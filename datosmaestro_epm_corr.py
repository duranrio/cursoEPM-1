...
selected_features = correlation_with_ENSA[
    abs(correlation_with_ENSA) > 0.8
].index.tolist()
...
features_selected_scaled_df = pd.DataFrame(
    features_selected_scaled, columns=selected_features
)
...
X, y = create_sequences(
    features_selected_scaled_df, datos_Maestro["ENSA"], window_size=12
)
...
history = model.fit(
    X_train, y_train, epochs=40, batch_size=32, validation_data=(X_val, y_val)
)
...
if len(X_val) >= 6:
    ...
    print(
        "Verificación de forma de last_window antes de expand_dims:", last_window.shape
    )
    ...
    print(
        "No hay suficientes datos en X_val para crear 'last_window' para predicciones"
    )

# Generar fechas futuras
last_known_date = datos_Maestro.index.max()
future_dates = pd.date_range(
    start=last_known_date + pd.DateOffset(months=1), periods=6, freq="MS"
)
...
predicted_data = pd.DataFrame({"fecha": future_dates, "ENSA_pred": predictions})
...
plt.plot(
    datos_Maestro.index, datos_Maestro["ENSA"], label="Historical ENSA", color="blue"
)
plt.plot(
    predicted_data.index,
    predicted_data["ENSA_pred"],
    label="Predicted ENSA for 2024",
    color="red",
    linestyle="--",
)
